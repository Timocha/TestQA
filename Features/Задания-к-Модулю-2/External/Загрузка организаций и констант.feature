...
@Описание: Экспортный сценарий загрузки констант для создания Заказа
@ПримерИспользования: И перезаполнению константы для создания заказа
Сценарий: перезаполнению константы для создания заказа
...
@ТипШага: Загрузка данных
@Описание: Экспортный сценарий загрузки организаций для создания Заказа
@ПримерИспользования: И перезаполнению организации для создания заказа
Сценарий: перезаполнению организации для создания заказа

	И я проверяю или создаю для справочника "Организации" объекты:
		| 'Ссылка'                                                                 | 'ПометкаУдаления' | 'Код'       | 'Наименование'        | 'ВалютныйУчет' |
		| 'e1cib/data/Справочник.Организации?ref=8d34000d8843cd1b11dd2bea12f94c63' | 'True'            | '000000001' | 'Помечен на удаление' | 'True'         |
		| 'e1cib/data/Справочник.Организации?ref=8d34000d8843cd1b11dd2bea12f94c64' | 'False'           | '000000002' | 'Валютный учет'       | 'True'         |
		| 'e1cib/data/Справочник.Организации?ref=8d34000d8843cd1b11dd2bea12f94c65' | 'False'           | '000000003' | 'Только рубли'        | 'False'        |
